...
Over the cumulative period of engagement, """
    ans += f"""the user have a total of {len(apps)} applications, listed as follows: {applist}. this figure, for all its apparent precision, excludes system applications that reside within protected directories, resistant to removal and {sysnum} of them are system. Number of custom apps that user has installed into systemm is {len(apps) - sysnum} and theese apps are {capplist}. This realization provokes a curious blend of emotions, for while such system-level programs are shielded by design, it remains within the realm of possibility for the user to bypass these safeguards, should they acquire the requisite knowledge—perhaps by consulting the CLIsma wiki. It is whispered that such a repository of wisdom might reside on a certain GitHub page, though the specifics remain shrouded in uncertainty. Who is to say what truths may lie therein?"""
    return ans
